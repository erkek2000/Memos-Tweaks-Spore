...
| Spacebar close confirmed in-game on 0.5.21 | Dialogue close shortcuts | On 2026-09-12, the user confirmed Space closes the planet's "Speak with the colony" dialogue, restores movement, zoom, and travel immediately, permits reopening, and leaves the Speak button steady. Still test ESC, TAB, and End, submenu confirmations, and unchanged key behavior outside communication. |
...
| Known failing in 0.5.13; 0.5.15 unverified | Planner building shortcuts | Current-build report: 1–4 do nothing in the colony planner. Test 0.5.15 and inspect `keyboard-input.log` for key receipt, planner state, palette-item matching, and dispatch; then test all four keys, unavailable items, out-of-planner behavior, and modifiers. |
| Pending | Space tool hotbar keys | Handler is present but not confirmed in-game. Test every Space tool tab. Unmodified 1–9 and 0 where a tenth slot exists must act only on the visible native panel and respect disabled, depleted, recharging, cargo, and tab-switching states. Modified keys must remain untouched; planner 1–4 must keep building priority. |
